...
class MakeTable:
    ...
    def package(self) -> dict[str,dict[str,dict[str,str|list]]|Table]:
        table = Table(title="What [yellow]Package[/yellow] do you want to [bold green]use[/bold green]?")        
        ...
        for i,theme_folder in enumerate(self.themes_folder):
            themes = list(theme_folder.rglob("*.conf"))
    ...
    def themes(self, themes: list, visited_theme: list) -> Table:
        table = Table(title=f"Select a [yellow]Theme[/yellow] to [red]preview[/red]")
        table.add_column("[bold cyan]ID[/bold cyan]")
        table.add_column("[bold cyan]Theme Name[/bold cyan]")

        themes.sort()
        for i,theme in enumerate(themes):
            if theme in visited_theme:
                table.add_row(
                    f"[green]{i+1}[/green]",
                    f"{theme.stem}",
                )
                ...
                table.add_row(
                    f"[green]{i+1}[/green]",
                    f"[bold blue]{theme.stem}[/bold blue]",
                )
